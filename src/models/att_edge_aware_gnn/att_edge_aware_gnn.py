...
from torch_geometric.nn import SAGEConv,GraphConv, global_mean_pool, GATv2Conv
...
class AttEdgeAwareGNN(nn.Module):
 ...
 #       print('output:', output_dim)
            ...
            # Substituição para GATv2Conv:
    # O parâmetro edge_dim permite que a atenção do nó use os atributos das arestas
 #       self.gc1 = GATv2Conv(node_input_dim, hidden_dim, edge_dim=edge_input_dim)
 #       self.gc2 = GATv2Conv(hidden_dim, hidden_dim, edge_dim=edge_input_dim)
        ## Node feature processing convolutional layers
        #self.gc1 = GraphConv(node_input_dim, hidden_dim)
        #self.gc2 = GraphConv(hidden_dim, hidden_dim)
        # Edge feature processing SAGE layer
 ...
 #       self.dropout = dropout
 #   def forward(self, node_features, edge_index, edge_features):
        # Aplica a primeira camada GATv2 passando também os atributos das arestas
 #       x = F.relu(self.gc1(node_features, edge_index, edge_attr=edge_features))
 #       x = F.dropout(x, self.dropout, training=self.training)

        # Aplica a segunda camada GATv2
 #       x = F.relu(self.gc2(x, edge_index, edge_attr=edge_features))
